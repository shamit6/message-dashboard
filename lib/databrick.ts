...
export type LineChartQueryResult = { time_bin: string; cnt: number }[];
...
export async function fetch(): Promise<{
  ...
  lineChartQueryResult: LineChartQueryResult;
  sentimentChartQueryResult: SentimentChartQueryResult;
  // sentimentOverTimeQueryResult: SentimentOverTimeQueryResult;
}> {
  console.log('Connecting to Databricks SQL...', connectOptions);
  return client
    .connect(connectOptions)
    .then(async (_client: IDBSQLClient) => {
      ...
      const [
        ...
        lineChartQuery(session),
        sentimentChartQuery(session),
        sentimentOverTimeQuery(session),
      ]);
      await session.close();
...
async function lineChartQuery(session: IDBSQLSession) {
  return query(
    session,
    `WITH time_bins AS (
    SELECT
...
ORDER BY
    tb.time_bin;`,
  ) as Promise<LineChartQueryResult>;
}
